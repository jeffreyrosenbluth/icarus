...
function tweakAngle(v: Vec, s: number): Vec {
  let angle = Math.atan2(v.y, v.x);
  angle += s * (Math.random() - 0.5);
  const r = v.mag();
  return new Vec(r * Math.cos(angle), r * Math.sin(angle));
}
...
export class Ball {
  ...
  move(gameState: GameState) {
    ...
    // Bounce the ball off the walls then tweak the angle so that balls don't
    // get trapped moving in lines parallel to the axes.
    if (this.pos.x < this.radius) {
      this.pos.x = this.radius;
      this.vel.x = -this.vel.x;
      this.vel = tweakAngle(this.vel, TWEAK);
    }
